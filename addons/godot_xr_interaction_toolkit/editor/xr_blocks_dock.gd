...
const CATEGORIES := [
	{"name": "Start Here", "blocks": [
		...
		{"name": "Remote Actuator (dev)", "desc": "Scripted XR input over a WebSocket: an operator drives poses, hands, gestures and sticks through the real interaction chain and streams every signal back - editor Play, on-device, or headless. Ship-inert: does nothing unless enabled AND the run names an endpoint (--operator= / ?operator=). The desktop simulator's input stands down the moment a drive writes; you can still hand-drive before the first scripted action.", "kind": "node", "base": "Node", "path": "res://addons/godot_webxr_kit/runtime/xr_remote_actuator.gd", "icon": "res://addons/godot_webxr_kit/icons/openxr_input_adapter.svg"},
	...
	{"name": "Hands & Input", "blocks": [
		{"name": "Grip Recording Station", "desc": "Record grips WITH THE OBJECT IN HAND. Give it a list of scenes; each is presented in turn, you wrap your hand round the real shape, and the snapshot saves under a name the matching grab point finds automatically. Objects without a recording still work - their grab point auto-fits the collider.", "kind": "node", "base": "Node3D", "path": "res://addons/godot_xr_hands/runtime/gesture_studio/xr_grip_recording_station.gd", "icon": "res://addons/godot_xr_hands/icons/xr_gesture_recognizer.svg"},
	...
	{"name": "Grab & Sockets", "blocks": [
		...
		{"name": "Pinch-Twist Distance Driver", "desc": "Drives a TWIST far-grab from wrist roll (needs godot_xr_hands - bare hands only). Roll away from the roll you grabbed with maps ABSOLUTELY to a distance offset from the distance you engaged at: twist one way to pull the object in, the other to push it out, and roll back to neutral to retrace to where you started - so pinch stays occupied grabbing while the wrist, still free, places the object. Tune twist_span_metres (how far the OUTWARD offset reaches, and the inward one too when the field below is off), twist_full_angle_degrees (the roll for full deflection), twist_start_threshold_degrees (roll before it engages at all) and twist_deadband_degrees (residual roll ignored once engaged). twist_pull_reaches_hand (default ON) makes full INWARD twist reach min_grab_distance - all the way to the hand - regardless of how far away the object was when TWIST engaged, instead of the fixed twist_span_metres offset (which used to land at very different fractions of the way in depending on engage distance); turn it off to use twist_span_metres inward too, symmetric with outward. Drop inside a Grabbable set to far_grab_mode = TWIST. Held by a controller (no wrist joint) it just holds its distance, like FIXED.", "kind": "node", "base": "Node", "path": "res://addons/godot_xr_interaction_toolkit/runtime/xr_pinch_twist_distance_driver.gd", "icon": "res://addons/godot_xr_interaction_toolkit/icons/xr_grab_interactable.svg"},
		...
		{"name": "Drawing Surface", "desc": "A notepad you draw on: give it a PlaneMesh and any Pen tip (group xr_pen_tip) paints ink where it touches. Runtime texture on a pre-baked material, so it works on WebGPU too. clear() wipes it.", "kind": "node", "base": "MeshInstance3D", "path": "res://addons/godot_xr_interaction_toolkit/runtime/xr_drawing_surface.gd", "icon": "res://addons/godot_xr_interaction_toolkit/icons/xr_ui_canvas_interactable.svg"},
	...
	{"name": "Perception (AR)", "blocks": [
		...
		{"name": "Synthetic Room", "desc": "Fake scanned room (meshes + semantic labels + collision) for desktop/editor debugging - the perception blocks work without a headset.", "kind": "node", "base": "Node3D", "path": "res://addons/godot_webxr_scene_understanding/runtime/xr_synthetic_room.gd", "icon": "res://addons/godot_webxr_scene_understanding/icons/scene_mesh_manager.svg"},
	]},
